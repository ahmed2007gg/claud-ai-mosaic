...
def _load_stats():
    """تهيئة الإحصائيات في الذاكرة فقط (Railway لا يحتفظ بالملفات)"""
    _init_stats()
...
def _save_stats():
    pass  # Railway: filesystem مؤقت — الإحصائيات في الذاكرة فقط
...
async def cmd_pause(msg: types.Message):
    ...
    if len(args) == 0:
        if is_paused():
            remaining = int(pause_until - asyncio.get_running_loop().time())
    ...
    pause_until = asyncio.get_running_loop().time() + secs
...
CHECKERS = {k: (lambda k: lambda: _check_center(k))(k) for k in CALENDAR_IDS}
    ...
    while True:
        ...
        for key in active:
            ...
            try:
                ...
                last_checked[key] = asyncio.get_running_loop().time()
